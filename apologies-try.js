...
function packageImports(nodes) {
    ...
    function bilink(root) {
        console.log(root);
        const map = new Map(root.map(d => [id(d), d]));
        for (const d of root) d.incoming = [], d.outgoing = d.data.imports.map(i => [d, map.get(i)]);
        for (const d of root) for (const o of d.outgoing) o[1].incoming.push(o);
        console.log(root);
    ...
    const val = bilink(root.leaves())
    const newRoot = tree(val);
