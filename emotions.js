d3.csv("data/emotions/EmotionsData-Planned-Details.csv", function (d) {
  ...
  for (var i = 0; i < NUM_EMOTIONS; i++) {
    ...
    svg
      .append("rect")
      .attr("x", XOFFSET_SQUARE + xSegment * 150)
      .attr("y", ySegment * 20)
      .attr("width", SIZE)
      .attr("height", SIZE)
      .style("fill", COLORS[i]);
    svg
      .append("text")
      .attr("x", XOFFSET_TEXT + xSegment * 150)
      .attr("y", ySegment * 20 + 5)
      .text(EMOTIONS[i])
  ...
  function mousemove(event, d) {
    ...
    joyText.html(d.joy.toFixed(3)).attr("x", 100).attr("y", 5);
    sadFocus.attr("cx", x(d.ep)).attr("cy", y(d.sadness));
    sadText.html(d.sadness.toFixed(3)).attr("x", 135).attr("y", 25);
    fearFocus.attr("cx", x(d.ep)).attr("cy", y(d.fear));
    fearText.html(d.fear.toFixed(3)).attr("x", 260).attr("y", 5);
    disgustFocus.attr("cx", x(d.ep)).attr("cy", y(d.disgust));
    disgustText.html(d.disgust.toFixed(3)).attr("x", 280).attr("y", 25);
    angerFocus.attr("cx", x(d.ep)).attr("cy", y(d.anger));
    angerText.html(d.anger.toFixed(3)).attr("x", 420).attr("y", 5);
  ...
  function updateEpisodeData(d) {
    ...
    let excerptsSectionTitle = document.querySelector(
      "#episodes-excerpts-section-title"
    );
